        ...
        if (Globals::MenuOptions::ESP) {
            ...
            if (pCNetworkPlayerMgr) {
                ...
                for (int i = 0; i < PlayerCount + 2; i++) {
                    ...
                    if (!Globals::ESPOptions::LocalPlayer) {
                    ...
                    if (distance <= Globals::ESPOptions::espmaxdist) {
                        ...
                        if (Globals::ESPOptions::Corner) {
                            RENDERER->DrawCornerBox(ImVec2(Ent2DPos.x + width / 2, Ent2DPos.y + height / 2), ImVec2(width, height), Globals::Colors::ESPCorner, 1);
                        }

                        if (Globals::ESPOptions::Snaplines) {
                            //RENDERER->DrawLine(LocPos2D, Head, WHITE, 2);
                        }

                        if (Globals::ESPOptions::Skeleton) {
                            ...
                            // BACK / SPINE
                            RENDERER->DrawLine(Head, Back, Globals::Colors::ESPSkeleton, 1);

                            // LEFT ARM
                            RENDERER->DrawLine(Back, LeftUpperArm, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(LeftUpperArm, LeftForearm, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(LeftForearm, LeftHand, Globals::Colors::ESPSkeleton, 1);

                            // RIGHT ARM
                            RENDERER->DrawLine(Back, RightUpperArm, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(RightUpperArm, RightForearm, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(RightForearm, RightHand, Globals::Colors::ESPSkeleton, 1);

                            // SPINE
                            RENDERER->DrawLine(Back, Pelvis, Globals::Colors::ESPSkeleton, 1);

                            // LEFT LEG
                            RENDERER->DrawLine(Pelvis, LeftThigh, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(LeftThigh, LeftCalf, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(LeftCalf, LeftFoot, Globals::Colors::ESPSkeleton, 1);

                            // RIGHT LEG
                            RENDERER->DrawLine(Pelvis, RightThigh, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(RightThigh, RightCalf, Globals::Colors::ESPSkeleton, 1);
                            RENDERER->DrawLine(RightCalf, RightFoot, Globals::Colors::ESPSkeleton, 1);
                        }

                        if (Globals::ESPOptions::Health) {

                            float HealthPercentage = height / 75 * (Health - 100);
                            float fixedhealth = Health - 100;
                            if (Globals::ESPOptions::HealthBarBasedOnHealth) {
                                ImColor col;
                                if (fixedhealth == 75) {
                                ...
                                RENDERER->DrawBox(ImVec2(Ent2DPos.x + (width / 1.865), Ent2DPos.y - height / 2), ImVec2(4, height), ImColor(40, 41, 40, 255), 1);
                            }
                            else {
                                if (Health > 0) {
                                    RENDERER->DrawHealthBar(ImVec2(Ent2DPos.x + (width / 1.87), Ent2DPos.y + height / 2), ImVec2(4, HealthPercentage), Globals::Colors::ESPHealthbar);
                                }
                                RENDERER->DrawBox(ImVec2(Ent2DPos.x + (width / 1.865), Ent2DPos.y - height / 2), ImVec2(4, height), ImColor(40, 41, 40, 255), 1);
                            }
                        }
                        if (Globals::ESPOptions::DarkBackground) {
                        ...
                        if (cped->pCWeaponManager) {
                            if (cped->pCWeaponManager->CWeaponCur) {
                                if (Globals::ESPOptions::Distance && Globals::ESPOptions::Weapon) {
                                    std::string text;
                                    ...
                                    RENDERER->DrawTextA(ImVec2((Ent2DPos.x - width / 2) + (width / 2 - font_size + (font_size / 2) + 20), Ent2DPos.y + height / 2), Globals::Colors::ESPDistance, text.c_str());
                                }
                                else if (Globals::ESPOptions::Weapon && !Globals::ESPOptions::Distance) {
                                    std::string text;
                        ...
                        if (Globals::ESPOptions::Box) {
                            RENDERER->DrawBox(ImVec2(Ent2DPos.x + width / 2, Ent2DPos.y - height / 2), ImVec2(width, height), Globals::Colors::ESPBox, 1);
                        }
                    }
                }
            }
